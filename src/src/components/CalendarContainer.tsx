...
function _CalendarContainer<T extends ICalendarEventBase>({
  ...
  const onHandlerStateChange = ({ nativeEvent }: any) => {
    ...
    if (nativeEvent.state === State.END) {
      ...
      //   return;
      // }
      if (scale.value >= 1.1 && reMode == 'week') {
        setMode('3days');
        scale.value = 1;
      } else if (scale.value >= 1.1 && reMode == '3days') {
        setMode('day');
        scale.value = 1;
      } else if (scale.value <= 0.99 && reMode == 'day') {
        scale.value = 1;
        setMode('3days');
      } else if (scale.value <= 0.99 && reMode == '3days') {
        scale.value = 1;
        setMode('week');
      } else if (scale.value <= 0.99 && reMode == 'week') {
        scale.value = 1;
        navigation.goBack();
      }
    }
  ...
  const slotStyle = useAnimatedStyle(() => {
    return {
      transform: [{ scaleY: scale.value > 1 ? scale.value : 1 }],
  ...
  const onSwipeHorizontal = React.useCallback(
    (direction: HorizontalDirection) => {
      ...
      if (onChangeDate) {
        const nextDateRange: any = getDateRange(nextTargetDate);
        onChangeDate([
          nextDateRange[0].toDate(),
          nextDateRange.slice(-1)[0].toDate(),
        ]);
      }
    },
    [
    ...
    <React.Fragment>
      ...
      >
        ...
        <Animatable.View duration={500} ref={viewRef}>
          ...
          >
            <Animated.View style={{ flex: 1, ...slotStyle }}>
              <CalendarBody
                {...commonProps}
                style={{ ...bodyContainerStyle }}
                containerHeight={height}
